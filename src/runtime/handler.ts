...
import type {
  FeedmeModuleContentOptions,
  FeedmeRSSContentOptions,
} from "../content";
import {
  createFeedFrom,
  createItemFrom,
  getItemOptionsFrom,
  mergeFeedmeContentOptions,
} from "./content";
import { intoParsedContentTemplateMapping } from "./defaults/mapping";
import {
  getFeedmeModuleOptions,
  getFeedmeRSSTypeFrom,
  intoContentType,
  intoSeconds,
} from "./feedme";
...
const feedmeHandleContent = async (
  event: H3Event,
  ...
  setHeaders(event, {
    "Content-Type":
      intoContentType(feedme?.type ?? getFeedmeRSSTypeFrom(event.path)) ??
      "text/plain",
  ...
  await useNitroApp().hooks.callHook(
    `feedme:handle:content:before[${event.path}]`,
    feedmeHandleContentBefore
  );
  await useNitroApp().hooks.callHook(
    "feedme:handle:content:before",
    feedmeHandleContentBefore
  ...
  for (const parsed of records) {
    ...
    await useNitroApp().hooks.callHook(
      `feedme:handle:content:item[${event.path}]`,
      feedmeHandleContentItem
    );
    await useNitroApp().hooks.callHook(
      "feedme:handle:content:item",
      feedmeHandleContentItem
    ...
    const feedmeItemParsedDefaults = getItemOptionsFrom(
      parsed,
      extendedFeedmeContentMapping
    );
  ...
  await useNitroApp().hooks.callHook(
    `feedme:handle:content:after[${event.path}]`,
    feedmeHandleContentAfter
  );
  await useNitroApp().hooks.callHook(
    "feedme:handle:content:after",
    feedmeHandleContentAfter
  ...
  if (!kind)
    return new H3Error(
      `[nuxt-feedme]: Unable to determine RSS feed type from route '${event.path}'`
    );

  if (typeof feed[kind] !== "function")
    return new H3Error(
      `[nuxt-feedme]: Incorrect kind '${kind}' of RSS feed type from route '${event.path}'`
    );
...
const feedmeHandleDefault = async (
  event: H3Event,
  ...
  if (!feed)
    return new H3Error(
      `[nuxt-feedme]: The RSS feed wasn't created for route '${event.path}'`
    );
...
export default defineEventHandler(async (event) => {
  ...
  if (!feedme) {
    return new H3Error(
      `[nuxt-feedme]: Incorrect handler set for route '${event.path}'` +
        ` That route is not found in feeds: ${JSON.stringify(
          moduleOptions.feeds
        )}`
    );
  }
  ...
  if (feedmeContent.content)
    return await feedmeHandleContent(
      event,
      feedme,
      moduleOptions.content ?? {}
    );
